...
from keyboards.default.users_btn import menu_btn, effects_btn, cancel_support_btn, davom_etish
...
async def show_effects_handler(message: Message):
    ...
    effects_btnn = await effects_btn(effects)

    if effects:
        await message.answer("Effectni tanlang:", reply_markup=effects_btnn)
...
async def get_selected_effect_handler(message: Message, state: FSMContext):
    ...
    if effect:
        cansel = await cancel_support_btn()
        await state.update_data(effect=effect[0]['effect'])
        await message.answer("Rasim yuboring:", reply_markup=cansel)
...
def register_users_py(dp: Dispatcher):
    ...
    dp.register_message_handler(support_message_state, state=UserStates.support_message)
